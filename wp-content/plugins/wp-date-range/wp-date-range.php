...
echo '});	
  ...
  public function date_range_where( $where) {
	...
	$where .= " AND (
	  ...
	  (
		CAST(postmeta_" . DATE_RANGE_FROM . ".meta_value AS UNSIGNED) <= CAST(UNIX_TIMESTAMP() AS UNSIGNED)
		AND
		CAST(postmeta_" . DATE_RANGE_TO . ".meta_value AS UNSIGNED) >= CAST(UNIX_TIMESTAMP() AS UNSIGNED)
	  )
	  OR
	  (
		CAST(postmeta_" . DATE_RANGE_FROM . ".meta_value AS UNSIGNED) <= CAST(UNIX_TIMESTAMP() AS UNSIGNED)
